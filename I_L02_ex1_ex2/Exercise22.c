...
void addSong(Music_Library *library){
    // Allocate memory for a new Music_Song struct
    Music_Song *newNode = malloc(sizeof(Music_Song));
    if (newNode == NULL) {
        printf("Memory allocation error.\n");
        return;
    }
    ...
    // Add the new song to the library by updating the head pointer and linking it to the previous head
